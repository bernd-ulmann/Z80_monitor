...
;
; 29-MAY-2013, B. Ulmann: Adapted for N8VEM and my own homebrew Z80 system.
;

auto_mem        equ     1       ; Do not ask for memory top if set to 1.
...
BCR     EQU     0DH             ; Carriage return; BU: Was CR
...
#if LOADABLE = 1
        ORG     $8000
; TODO: Somehow I am unable to determine the end of the BASIC interpreter's
;       code which would be necessary for determining the value for WRKSPC
;       automatically. Thus the ORG directive and the magic constant $2000
;       are necessary until I find a suitable workaround.
WRKSPC  equ     $ + $2000 ; BASIC Work space
#else
WRKSPC  equ     ram_start ; BASIC Work space
#endif
...
DTYPE   EQU     WRKSPC+0ADH           ; Data type flag ; BU: Was TYPE
...
CLDFLG  EQU     WRKSPC+0F9H           ; Flag for CLOAD
PROGST  EQU     WRKSPC+0FAH           ; Start of program text area
STLOOK  EQU     WRKSPC+15EH           ; Start of memory test
...
STARTB: 
    ; DI                      ; No interrupts
        LD      A, 0            ; Reset the CLDFLG for keyboard input
        LD      (CLDFLG), A
...
SETTOP: DEC     HL              ; Back one byte
        ...
        LD      HL, last_user_ram
#endif
        LD      DE,0-512        ; 50 Bytes string space
...
BFREE:  DEFB      " Bytes free",BCR,0,0

SIGNON: DEFB      "Z80 BASIC Ver 4.7b",BCR
        DEFB      "Copyright ",40,"C",41
        DEFB      " 1978 by Microsoft",BCR,0,0
...
BWORDS: DEFB      'E'+80H,"ND"  ; BU: Was WORDS
        DEFB      'F'+80H,"OR"
        DEFB      'N'+80H,"EXT"
        DEFB      'D'+80H,"ATA"
        DEFB      'I'+80H,"NPUT"
...
FNDWRD: PUSH    DE              ; Look for reserved words
        LD      DE,BWORDS-1     ; Point to table; BU: Was WORDS
        PUSH    BC              ; Save count
...
; BU: Enclose deleted characters in /\ and echo them:
        PUSH    AF
        LD      A, '/'
        CALL    OUTC            ; Output character in A
        LD      A, (HL)         ; Get character that was deleted
        CALL    OUTC
        LD      A, '\'
        CALL    OUTC
        POP     AF
; BU: End of deleted characters echo.
...
LISTLP: POP     HL              ; Restore address of line
        ...
        CALL    LCOUNT          ; Count lines; BU: Was COUNT
...
LSTLP3: LD      A,(HL)          ; Get next byte in line
        ...
        LD      DE,BWORDS       ; Reserved word list; BU: Was WORDS
...
LCOUNT: PUSH    HL              ; Save code string address; BU: Was COUNT:
        PUSH    DE
...
; 20-OCT-2013: B. Ulmann: Get rid of the blockwise output in listings:
;        CALL    GETINP          ; Get input character
        CP      CTRLC           ; Is it control "C"?
        JP      Z,RSLNBK        ; Yes - Reset LINES and break
        POP     HL              ; Restore code string address
        JP      LCOUNT          ; Keep on counting; BU: Was COUNT
...
;   RST     10H     ; Get the key into A
        ...
        push    ix
        ld      ix, _uart_status
        rst     08              ; Perform system-call to get RxD-status
        pop     ix
...
LET:    CALL    GETVAR          ; Get variable name
        ...
        LD      A,(DTYPE)       ; Get data type; BU: Was TYPE
        PUSH    AF              ; Save type
...
BIF:     CALL    EVAL           ; Evaluate expression; BU: Was IF:
        LD      A,(HL)          ; Get token
        CP      ZGOTO           ; "GOTO" token?
...
PRNTLP: RET     Z               ; End of list - Exit
        ...
        CALL    EVAL            ; Evaluate expression
        PUSH    HL              ; Save code string address
        LD      A,(DTYPE)       ; Get variable type; BU: Was TYPE
...
PRNTCR: LD      A,BCR           ; Load a CR; BU: Was CR
        CALL    OUTC            ; Output character
...
REDO:   DEFB      "?Redo from start",BCR,LF,0
...
GTVLUS: CALL    GETVAR          ; Get variable name
        ...
        PUSH    DE              ; Save variable address
ANTVLU: LD      A,(DTYPE)       ; Check data type; BU: Was TYPE
        OR      A               ; Is it numeric?
...
EXTIG:  DEFB      "?Extra ignored",BCR,LF,0
...
CHKTYP: LD      A,(DTYPE)       ; Check types match; BU: Was TYPE
...
OPRND:  XOR     A               ; Get operand routine
        LD      (DTYPE),A       ; Set numeric expected; BU: Was TYPE
        CALL    GETCHR          ; Get next character
        LD      E,MO            ; ?MO Error
    ...
    JP  Z,BINTFP    ; Convert Bin to FPREG
        ...
        CP      ZMINUS          ; '-' Token ?
        JP      Z,BMINUS        ; Yes - Do minus; BU: Was MINUS
        CP      '"'             ; Literal string ?
...
BMINUS: LD      D,7DH           ; '-' precedence; BU: Was MINUS:
...
FRMEVL: PUSH    HL              ; Save code string address
        ...
        LD      A,(DTYPE)       ; Get type; BU: Was TYPE
        OR      A               ; Numeric?
...
CMPLG1: LD      A,C             ; Get data type
        ...
        LD      (DTYPE),A       ; Set type to numeric; BU: Was TYPE
...
CHARTY: SUB     '$'             ; String variable?
        ...
        LD      (DTYPE),A       ; Set type to string; BU: Was TYPE
...
FRE:    LD      HL,(ARREND)     ; Start of free memory
        ...
        LD      A,(DTYPE)       ; Dummy argument type; BU: Was TYPE
        OR      A
...
ABPASS: LD      D,B             ; Return integer AB
        LD      E,0
        LD      HL,DTYPE         ; Point to type; BU: Was TYPE
...
BABS:   CALL    TSTSGN          ; Test sign of FPREG; BU: Was ABS:
...
CONMON: PUSH    HL              ; Output character to screen
        ...
        CP      BCR              ; ASCII CR?; BU: Was CR
        JP      NZ,OUTCHR       ; No - Output character
        LD      A,BCR               ; CR; BU: Was CR
...
GETINP:
;
; BU: The CLOAD command which loads a BASIC program from a FAT16 disk 
;     manipulates the input channel which, in turn, is controlled by 
;     the CLDFLG flag. If this is set to 0, input is expected from the
;     keyboard; otherwise input is from the file. When EOF is reached,
;     CLDFLG is reset and keyboard input resumes.
;
        ld      a, (CLDFLG)     ; Determine the input mode (keyboard / file)
        cp      0
        jr      z, KBDINP       ; Read from keyboard
        ld      iy, fcb         ; First address of FCB for fgetc()
#if LOADABLE = 1
        push    ix
        ld      ix, _fgetc      ; Read from file
        rst     08
        pop     ix
#else
        call    fgetc
#endif
        jr      c, GETINP_EOF   ; EOF found?
        cp      0
        jr      z, GETINP_EOF   ; EOF found?
        cp      LF              ; A UNIX-like LF must be converted into CR!
        ret     nz              ; No CR found, just return character read
        ld      a, BCR          ; Return a CR character explicitly
        ret
GETINP_EOF:                     ; We are done reading the file
        ld      a, 0            ; Reset the CLDFLG
        ld      (CLDFLG), a
        ld      a, BCR          ; Return a CR character
        ret
KBDINP: 
#if LOADABLE = 1
        push    ix
        ld      ix, _getc       ; Input from keyboard
        rst     08
        pop     ix
#else
        call    getc
...
; BU: Load a program from a FAT16 disk:
CLOAD:  PUSH    AF
        PUSH    BC
        PUSH    HL
        LD      A, 1                ; Set CLDFLG to redircect input to file
        LD      (CLDFLG), A
        LD      HL, CLOAD_PROMPT    ; Prepare output of prompt string
#if LOADABLE = 1
        PUSH    IX
        LD      IX, _puts
        RST     08
        LD      IX, _uart_status    ; If there is a garbage CR/LF, remove it
        RST     08
        JR      NC, CLOAD_1         ; No, just continue reading the filename
        LD      IX, _getc
        RST     08        
CLOAD_1:
        LD      HL, string_81_bfr   ; Buffer for filename
        LD      B, $d
        LD      IX, _gets           ; Read filename
        RST     08
        PUSH    HL
        LD      HL, CLOAD_MSG_1     ; Print "Loading"-message followed by...
        LD      IX, _puts
        RST     08
        POP     HL                  ; ...the filename
        LD      IX, _puts           ; Beware, IX is NOT preserved at a sys call!
        RST     08
        LD      IX, _crlf
        RST     08
        LD      DE, string_12_bfr   ; Setup scratch areas for fopen()
        LD      IY, fcb
        LD      IX, _fopen
        RST     08                  ; Open file
        POP     IX
...
CLOAD_OK:
        ...
        POP     BC
        POP     AF
;        JP      WARMST              ; Warmstart the interpreter
;        RET
;
        POP     IY
        JP      PRNTOK
CLOAD_PROMPT    DEFB    "Filename: ", eos
CLOAD_MSG_1     DEFB    "Loading ", eos

; HEX$(nn) Convert 16 bit number to Hexadecimal string

BHEX:   CALL    TSTNUM      ; Verify it's a number; B. Ulmann: Was HEX:
    CALL    DEINT       ; Get integer -32768 to 32767
...
; BIN$(NN) Convert integer to 16 char binary string
...
; Convert "%nnnn" to FPREG
; Gets a character from (HL) checks for Binary ASCII numbers "%nnnn"
